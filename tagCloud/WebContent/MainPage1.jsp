...
function changeInfo(f)
{
	...
	document.getElementById('wikiFrame').style.left=event.clientX+50;
	document.getElementById('wikiFrame').style.top=event.clientY+10;
	document.getElementById("wikiFrame").src="wordInfo.jsp?word="+f;
...
function clear()
{
	...
	//	document.getElementById('wikiFrame').style.left=event.clientX+5000;
	//document.getElementById('wikiFrame').style.top=event.clientY+1000;
...
%>
   ...
   <tr><td class="menu"><a onmousedown="newPopup('http://en.wikipedia.org/wiki/<%=r.getString(1)%>')"><%=r.getString(1)%></a></td></tr>
...
<%
	...
	try {
		Statement s=c.createStatement();
		ResultSet r=s.executeQuery("select * from words where word in (select word from userwords where username='"+session.getAttribute( "userName" )+"')");
		while (r.next()) {
			
%>
				<h2 class="glidetext" onmouseout="clear()" onmouseover="changeInfo('<%=r.getString(1)%>')">	
				<%=r.getString(1)%>				
				</h2>   
   <%
...
<iframe id="wikiFrame" frameborder="0" style="visibility:visible">
</iframe>
